...
export default function ForgotPassword() {
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    try {
      await AuthService.forgotPassword(loginId);
      setSubmitted(true);
    } catch (err: any) {
          ...
          <AnimatePresence mode="wait">
            {!submitted ? (
              ...
              >
                <Link
                  to="/login"
                  className="flex items-center gap-2 text-[10px] uppercase tracking-widest font-bold text-brand-navy/40 hover:text-brand-navy mb-8 transition-colors"
                >
              ...
              >
                ...
                <p className="text-brand-navy/50 text-sm leading-relaxed mb-10">
                  We've sent a secure password reset link to your email. Please check your
