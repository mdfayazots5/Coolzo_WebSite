...
import type { SupportTicketLookupItem } from "../../types/ticket";
...
export default function NewTicket() {
  ...
  const [categories, setCategories] = useState<SupportTicketLookupItem[]>([]);
  const [priorities, setPriorities] = useState<SupportTicketLookupItem[]>([]);
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      if (priResult.status === 'fulfilled') {
        const pris = priResult.value ?? [];
        setPriorities(pris);
        if (pris.length > 0) setForm((f) => ({ ...f, priorityId: pris[0].id }));
      }
    });
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.subject.trim() || !form.description.trim() || form.categoryId === '' || form.priorityId === '') return;
    ...
    try {
      ...
      setCreatedTicketId(ticket.supportTicketId);
    ...
    return (
      <div className="max-w-xl mx-auto py-12 md:py-20 text-center px-4">
        ...
        >
          ...
          <p className="text-[10px] uppercase tracking-widest font-bold text-brand-gold mb-6">
            {createdTicketNumber}
          </p>
          <p className="text-brand-navy/50 text-sm mb-10 leading-relaxed">
            Your support request has been received. Our team typically responds within 4 working hours.
            You can track the progress in your tickets list.
          </p>
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                >
                  {categories.length === 0 ? (
                    <>
                      <option value={1}>Booking Issue</option>
                      <option value={2}>Technician Concern</option>
                      <option value={3}>Invoice Query</option>
                      <option value={4}>AMC Query</option>
      ...
      <div className="mt-12 p-8 bg-brand-navy/5 rounded-sm flex gap-6">
        ...
        <div>
          ...
          <p className="text-xs text-brand-navy/60 leading-relaxed">
            If you are experiencing an emergency (e.g. electrical fire or major flooding), please call our 24/7
            Priority Concierge at <span className="text-brand-navy font-bold">+91 1800-COOLZO</span>.
          </p>
        </div>
      </div>
    </div>
